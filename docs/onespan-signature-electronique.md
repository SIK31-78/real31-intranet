...
## 3. eStale — où OneSpan complète (ne remplace pas)
- **eStale couvre le BUREAU seul** : enum `MeetingPersona` = `CHAIRMAN/SECRETARY/SCRUTINEER`. Mécanismes = **upload d'un fichier** (`file: Upload`) : `createSigning` (émargement bureau), `createTranscriptDigitalSignature(persona, file)` + `transcript.setBody(file)` / `signedBodyURL` / `froze`. → « signature » eStale = on **téléverse une image** rattachée à une persona, **pas** de cérémonie cryptographique ni preuve eIDAS.
- **eStale NE couvre PAS les présents** : `MeetingAttendance.isSigned` = simple statut oui/non ; aucune mutation de capture manuscrite d'un présent ; `attendanceSheetURL` = PDF généré → aujourd'hui imprimer→signer→scanner (**le flux à tuer**).
- **Donc** : OneSpan = moteur de cérémonie in-person (présents) + preuve eIDAS ; les docs signés **reviennent dans eStale** via les `Upload` (`setBody`/`createTranscriptDigitalSignature`). **eStale reste la source primaire.**
