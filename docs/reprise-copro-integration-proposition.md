...
- **Axe 2 (RÉVISÉ 2026-07-01, après lecture des procédures métier du vault)** : **PAS le moteur de wizard branché** du sinistre. La reprise est une **procédure LINÉAIRE** (pas un arbre de décision), et l'IA extrait tout des documents. Parcours = **fiche-hub pilotée par l'IA** : créer la copro (nom + n° entité S0XXX + adresse) -> l'IA extrait **cadrage + patrimoine** des documents -> l'humain **VÉRIFIE** (coche) -> injection eStale -> **checklist humaine** pour ce que l'IA ne fait pas (vérif V1-V4, finalisation P5, compta C1-C6, clôture). Le patrimoine = **action IA + résultats affichés**, jamais des cases manuelles. Suivi sur le `dossier.ts` de reprise (voie B-β), unification intranet plus tard. *Pourquoi le changement : le workflow Obsidian de Sekou est un template de suivi linéaire, pas un branchement ; coller le moteur de wizard aurait été surdimensionné et infidèle.*
