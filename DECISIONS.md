...
**`draft: true` par défaut.** Aucune facture n'est finalisée automatiquement : la validation reste un geste humain côté comptabilité. Le jeton Pennylane vient de `PENNYLANE_API_KEY` (environnement), **jamais** d'une table applicative - le legacy le stockait en clair dans la liste SharePoint « Paramètres ».

> **Amendement du 2026-09-04** - le « non négociable » devient un **opt-in explicite**. Demande du pôle comptable : que les factures partent directement en *facture validée*. L'API externe v2 le permet en **deux temps** (`POST /customer_invoices` crée toujours un brouillon, puis `PUT /customer_invoices/{id}/finalize` le valide) ; il n'existe pas de création directe en validé. C'est câblé derrière `PENNYLANE_FACTURE_VALIDEE=oui` (`adapters/pennylane/options.ts`), **défaut inchangé = brouillon**, parce que la finalisation est **irréversible** (*« Once finalized, the resource can no longer be edited »*) : la facture prend un numéro, engage la comptabilité, et ne se supprime plus. Le port s'appelle désormais `emettreFacture` (et non plus `creerFactureBrouillon`) et renvoie `validee` : le nom mentait dès lors que la méthode pouvait finaliser. Si la validation échoue après création, l'erreur **nomme le brouillon déjà créé** - rejouer l'émission sans le traiter à la main produirait un doublon.
...
### Précisions à la livraison (v2, 15-16/09/2026 — les trois briques sont codées)

7. **L'immatriculation au registre est LA clé d'un immeuble** (Sekou : « ce qui ne change pas chez une copro, c'est pas l'adresse mais l'immatriculation »). Elle donne l'historique (« déjà consulté en 2018, refusé ») et le lien avec la copro App A qui porte la même (247 copros sur 265 en ont une, 235 retrouvées au registre). **Rapprochement des adresses de l'Excel** (`domain/proposition/rapprochement.ts`) : automatique quand **numéro + voie + commune coïncident et qu'un seul immeuble répond** (615 sur 1 161), en tenant compte des adresses complémentaires du registre ; « 52 ter » contre « 52 », rue contre boulevard, commune absente → un humain choisit (`/propositions/a-rapprocher`, ou la fiche). Une autre commune n'est jamais proposée.
8. **Le prix garde la grille comme base** ; le gestionnaire applique un **geste commercial** en euros (négatif = majoration), le retenu se recalcule. **Frais postaux au réel par défaut** sur une offre (à l'inverse du contrat de renouvellement, forfait par défaut). Le syndic actuel n'est pas requis pour faire l'offre ; l'assurance du syndicat, si inconnue, laisse la phrase du contrat sans trou (« Titulaire d'un contrat d'assurance responsabilité civile »).
9. **L'offre (brique 2)** = le mail du cabinet pré-rédigé (montants, lieu d'AG de l'agence, signature) à copier dans Outlook + le **contrat prospect** sur le gabarit 2026 (durée 1 an / 2 ans / 15 mois, début au lendemain du mandat en place quand le registre le donne), imprimable comme `/contrat`. Les PDF fixes (présentation, démarches, courrier de résiliation) se joignent à la main pour le moment. « Offre envoyée » date la remise.
10. **L'élection (brique 3) est un bouton, jamais un changement de statut** : « Élue : créer la copropriété » relit code (prochain SXXX libre, App A et eStale confondus), nom court, agence, gestionnaire, prise en gestion, durée, puis crée dans l'ordre la fiche App A (`dataSource: ESTALE`, même forme que S302), le cycle de contrat tarifs figés, le client Pennylane (`POST /customers`, référence externe UUID gardée dans `Copropriete.pennylaneId`) et le dossier de reprise. Chaque étape va au journal ; un échec en route laisse les étapes faites et le dit. Nouveaux ports : `CoproRepository.creerCopro`, `InvoicingProvider.creerClient`.
11. **Une barre de recherche plutôt que des listes déroulantes** (règle générale de Sekou) : le pipeline se filtre par un texte libre qui couvre aussi agence, gestionnaire, origine, statut, année ; les compteurs suivent le filtre ; le **taux de transformation se mesure sur 3 ans glissants** (au-delà, l'Excel a été nettoyé : tout en refusé).
12. **Le barème annuel s'administre dans `/admin/tarifs`** (super-admin) : par famille, ouverture d'une année par copie de la précédente, majorée ou non. Les tarifs figés aux contrats enregistrés ne bougent pas.

### Conséquences

**Positives** : une copro gagnée est branchée le jour de l'élection, plus jamais découverte facturable-nulle-part trois mois après ; le prix se calcule au lieu de se recopier ; le registre donne les lots et le syndic sortant sans visite ; le taux de transformation par origine et par agence devient lisible. **Négatives / dettes** : 85 000 lignes de registre (92/78/95/75, ~45 Mo) à rafraîchir chaque trimestre à la main (`scripts/importer-registre-copros.mjs`, pas d'API ; la page dit la date du chargement, orange passé 4 mois) ; l'écriture dans App A depuis l'intranet s'étend (statut et dates hier, création de fiche demain — le schéma Prisma reste celui du patron, à border avec lui) ; les rôles (qui peut passer « élu », qui peut remiser) ne sont pas gérés.
...
## ADR-040 - Module « Gestion des clés » : un fait par vérité, un journal qu'on ne réécrit pas

**Date** : 2026-09-18 - **Statut** : accepté (Sekou, 2026-09-18 — 4 arbitrages : entreprise autonome, Storage dès l'import, pas d'accès externe en V1, trousseau + composition)

### Contexte

Le dernier outil MYTHEC à porter n'est pas un flux mais une **canvas app PowerApps** (« Gestion des Clés », 5 listes SharePoint, 10 écrans, format téléphone), utilisée par **l'agence LGC seule** : 167 trousseaux couvrant 134 des 141 copros actives, 2 391 mouvements en deux ans, 3 collaboratrices de l'accueil pour 65 % des saisies. L'audit (`docs/audit-gestion-des-cles-2026-09-18.md`) montre ce que l'outil ne sait pas dire : **37 trousseaux sont sortis aujourd'hui, dont 9 depuis plus de 240 jours**, sans qu'aucun écran le signale, faute de date de retour ; le journal n'est pas chaîné (chaque geste écrit une ligne isolée, 57 anomalies de séquence) ; le statut du trousseau est écrasé à chaque action et éditable à la main ; le collaborateur est choisi dans une liste, pas déduit de la session ; « Remettre » veut dire « l'entreprise nous rend les clés » ; 13 % des mouvements vont à une société « DIVERS ». Une solution commerciale (Keiko, 25-30 €/agence/mois, transcript `SPK_1.txt`) a été étudiée : date limite, relances graduées, contrôle de complétude au retour, attestation, NFC au comptoir — mais des comptes partagés qui ne disent pas qui a donné les clés.

Sekou a demandé une **V2 repensée**, pas une copie : comptoir en quelques secondes, « où sont les clés de cette copro » immédiat, traçabilité opposable, sans multiplier ni notifications ni portails.

### Décision

1. **Vocabulaire** : *trousseau* (l'unité prêtée, avec sa *composition* : clés, badges, bips, quantités), *accès* (copro obligatoire + immeuble facultatif + types), *entreprise* et *contact*, *réservation*, **« Sortir »** (remise à l'entreprise) et **« Enregistrer le retour »**. « Emprunter », « Remettre », « Confier » abandonnés. Le trousseau reste l'unité : **pas de suivi unitaire des clés**, la composition déclarée sert au contrôle du retour.
2. **Un seul fait stocké par vérité.** Le trousseau ne porte aucun état « sorti / réservé / en retard » : ils se **dérivent** du prêt ouvert (`rendu_le is null`), de la date de retour prévue et des réservations, par une fonction pure du domaine. Seules les marques qu'aucun prêt ne déduit (*introuvable*, *retiré*) sont stockées. Une réservation n'est jamais « expirée » en base : c'est `prevue` avec un début passé. La réservation est un **souhait indépendant de l'état physique** ; le conflit se règle à la sortie (refus sauf confirmation motivée si une autre entreprise a réservé aujourd'hui).
3. **Prêt + journal immuable**, pas de `journal jsonb`. `intranet_cles_pret` = le cycle sortie → retour (un index partiel unique garantit **un seul prêt ouvert par trousseau**, snapshots du contact et de la composition figés à la sortie, date de retour prévue obligatoire, défaut aujourd'hui, retard dès le lendemain). `intranet_cles_mouvement` = toute écriture datée par le serveur avec l'auteur de session ; **un trigger interdit `update` et `delete`** — `service_role` contourne les `revoke`, pas un trigger. Corriger = nouveau mouvement `correction` (direction, motif) qui référence l'original. Pattern nouveau dans le repo, assumé : c'est ce qui rend « qui avait les clés le 12 mars » opposable.
4. **Entreprise = référentiel cabinet autonome** (`intranet_cles_entreprise`, nom normalisé unique, contacts, blocage motivé, opt-out relances), **lié à ESTALE par `estale_supplier_id` facultatif**. C'est une **dérogation à ADR-022** (ESTALE source primaire), assumée parce que `SupplierCondo` est par copro alors que l'emprunteur est transverse (ABSOLU SERVICES sur 40 copros), qu'un artisan ponctuel n'est pas un fournisseur contractuel, et que le comptoir doit créer une entreprise en dix secondes sans dépendre d'ESTALE. L'enrichissement (contacts, contrats, bâtiments `condo.buildings`) viendra par des ports de lecture, plus tard. Prêt **interne** (gestionnaire en visite) = type de prêt sans entreprise, exclu des statistiques de retard.
5. **Multi-agence par conception, LGC seule à l'usage** : `agence_code` sur le trousseau, agence de session = `Gestionnaire.agencyId` ; tout collaborateur de l'agence opère (réserver, sortir, retour, prolonger, créer une entreprise), la **direction seule** corrige, bloque une entreprise, retire un trousseau, force une sortie vers une entreprise bloquée. Pas de cloisonnement par gestionnaire : le comptoir sert toutes les copros de l'agence. Aucune exposition `/api/v1` pour l'instant ; si elle vient, lecture seule (ADR-033).
6. **Photos dans Supabase Storage dès l'import** (bucket privé `cles`, URLs signées courtes servies par le serveur, SDK confiné à l'adapter Supabase) : 163 des 167 trousseaux ont une photo, la perdre serait la première régression visible au comptoir. **Première brique de stockage de fichiers du projet.** Repli si l'export des pièces jointes SharePoint par Graph échoue : photos reprises à la main au pointage.
7. **Aucun accès externe en V1.** Ensuite des **mails sortants** best-effort (confirmation de réservation, rappel J-1, relances J+1 / J+7 / J+15 puis escalade au collaborateur), tracés en mouvement `relance`, portés par le **premier cron du projet** (`src/lib/jobs/cles-relances.ts`, appelable par la route cron et en CLI, ADR-004). Un lien signé par réservation (pattern fiche de renseignements) reste une option V3 ; un portail avec comptes est **écarté** : il exposerait les accès des immeubles (TGBT, toiture) à des tiers pour quelques appels évités.
8. **Recherche** : l'index trousseaux + copros + entreprises (< 50 Ko) se charge une fois et se filtre côté client avec désaccentuation et tolérance (« r4 » → R004), comme la palette Ctrl+K ; journal et historiques paginés serveur. Les volumes ne justifient pas une recherche serveur sur les trousseaux.
9. **Migration** : script idempotent en deux passes (référentiel + photos, puis historique), 188 entreprises utilisées importées sur 2 036 (« DIVERS » devient « Non identifiée (historique) », bloquée), reconstruction des 1 033 cycles et des 37 prêts ouverts avec les incohérences marquées, **pointage physique des 37 par LGC** avant l'ouverture ; PowerApps en lecture seule le jour J, retiré du menu deux semaines après.

### Conséquences

**Positives** : l'agence sait enfin qui a quoi, depuis quand, et ce qui est en retard ; l'auteur et l'heure sont vrais ; un mouvement passé ne disparaît plus ; la copro est celle du référentiel (adresse, gestionnaire, agence, fiche copro enrichie, lien avec l'étape TR2 d'une perte) ; ML, HLS et ASN peuvent démarrer sans migration ; zéro abonnement. **Négatives / dettes** : deux patterns nouveaux à entretenir (trigger d'immuabilité, Storage) ; une dérogation à ADR-022 à revisiter quand ESTALE portera les tiers de façon transverse ; supabase-js n'offre pas de transaction — l'ordre d'écriture (prêt puis mouvement) est fixé et testé, un contrôle direction signale un prêt sans mouvement, une fonction RPC viendra si une écriture partielle est un jour observée ; le cron est une brique à créer avant les relances. Ce qui n'est pas tranché : la lecture des bâtiments ESTALE pour l'immeuble (libellé libre en V1), les questions terrain listées au § 18 de l'audit. **Gardé en tête, pas construit** (Sekou, 18/09) : la gestion locative et la transaction ont aussi des trousseaux ; le modèle ne doit pas le bloquer — l'accès porte une copro obligatoire en V1, l'extension prévue est un couple `bien_type` (copro / lot_locatif / bien_vente) + `bien_ref` avec `copropriete_id` rendu facultatif, sans toucher aux prêts ni au journal.
