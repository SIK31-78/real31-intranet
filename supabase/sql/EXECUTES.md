...
| `intranet_recap_ag_traitement.sql` | ✅ | **PASSÉ le 2026-08-17** (API de management Supabase, projet `lgrsnrclufsulglbwcqi`). Contrôle mesuré au retour : `recaps=0, a_traiter=0, traites=0` — la table est vide, cohérent avec le fait qu'aucun récap n'avait jamais pu être enregistré avant `22239f8`. Colonnes `traite_compta_at` / `traite_compta_par` sur `intranet_recap_ag` (marqueur « traité » de la file « Récaps d'AG reçus » de l'espace comptable). **Vérifié absentes en base le 2026-08-17** (`select=traite_compta_at` → `400 / 42703`). ⚠️ `create table if not exists` ne les rattrape pas : ce fichier est un `alter table ... add column if not exists`, idempotent. **Sans lui l'app tourne** : la file s'affiche, tout apparaît « à traiter », et le bouton « marquer traité » remonte une erreur explicite nommant ce fichier (pas de succès mensonger). ⚠️ Ne PAS détourner `notif_comptable_at` (elle trace un envoi de mail, jamais branché). |
