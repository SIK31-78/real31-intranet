...
- **Prochaine action concrète** : (contenu) **ODJ-3 export Word/PDF** (doc-gen, rouvre ADR-012) quand Sekou le décide, ou backlog MYTHEC (facturation dépassement). (Expo, quand voulu) infra seulement : **`db pull` côté patron** (consigne prête, désormais pour **3 tables**) + **déploiement Vercel** (différé). (Plus tard) **Entra ID** (remplace le sélecteur dev + débloque mails/réservation de salle), durcissement (RLS si possible, tests E2E).
