...
- **EN COURS - attribution copro "B sans eStale"** : les 28 non-rattaches = vraie limite (l'attribution ne matche que code/nom dans objet+corps). Comme les copros pilote **ne sont pas sur eStale**, on ne peut pas faire `expediteur -> coproprietaire (eStale) -> copro`. Solution : **cascade de resolveurs** (1er qui trouve gagne) sans eStale : (1) **carnet d'expediteurs appris** (memorise `expediteur -> copro` a chaque rattachement/classement, table native, s'ameliore avec l'usage) ; (2) **heritage par fil** (un mail herite de la copro d'un autre mail du meme fil, gratuit) ; (3) **match adresse** de l'immeuble (deja dans Supabase) ; (4) **import du gros JSON Crypto** (annuaire identites+emails -> table `intranet_crypto_contacts` indexee par email -> `expediteur -> copro` immediat, **LE gros levier**, fichier PII a deposer git-ignore) ; (5) **eStale plus tard** = un resolveur de plus. **Prochaine action** : Sekou fournit la **structure du JSON Crypto** (champs + email->copros multiple ?) + nb d'enregistrements ; en // attaquer **heritage par fil + match adresse** (sans SQL). Puis : items d'audit (fuite contexte eStale, garde choisirGestionnaire, "Demo", logs PII) et **apprentissage du ton ELRON** (priorite qualite : reponses trop generiques).
