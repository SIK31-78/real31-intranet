...
- **Prochaine action concrète** : (contenu) **focus ODJ = brique clé du MVP** (décision Sekou 2026-06-13). Fait : vue imprimable (PDF navigateur) + **aperçu live repliable** dans l'édition. À venir sur l'ODJ : **ODJ-4 mise en forme du document** (typo, logo REAL31, en-tête/pied de page, marges - le rendu est un composant à nous, entièrement stylable ; en option ultérieure : génération sur le modèle Word tagué pour une fidélité totale) ; puis export Word si besoin d'édition post-génération. Sinon : backlog MYTHEC (facturation dépassement). (Expo, quand voulu) infra seulement : **`db pull` côté patron** (consigne prête, désormais pour **3 tables**) + **déploiement Vercel** (différé). (Plus tard) **Entra ID** (remplace le sélecteur dev + débloque mails/réservation de salle), durcissement (RLS si possible, tests E2E).
