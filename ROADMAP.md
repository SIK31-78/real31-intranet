...
- **Prochaine action concrète** : app entière réelle **et cloisonnée** (sélecteur dev). Prochains chantiers : **Entra ID** (remplace le sélecteur dev par la vraie auth M365), **durcir le cloisonnement** (scope des écritures + RLS/vues si possible), **tests E2E** (Playwright). En parallèle (hors code, important) : **confirmer le `prisma db pull` avec le patron** pour les 2 tables natives (border le drift).
