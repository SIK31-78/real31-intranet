...
- **Derniers incréments terminés** :
  ...
  - ✅ **LIEN ESTALE <-> INTRANET BRANCHÉ EN RÉEL (2026-06-12, Phase B ADR-022, J4 anticipé)** : c'est le **socle de lecture** de la plateforme. Détails :
    ...
    - **E4 fait (2026-06-12)** : `constructionDate` -> **applicabilité PPT (>15 ans) / DPE (<2013) décidée automatiquement** ; **contrats gaz/élec** (`ENERGY_GAS`/`ENERGY_ELECTRICITY`, dates) auto-remplis dans la gestion ; **procédures** = `litigation.count`. Vérifié de bout en bout sur **SE999** (immeuble 2011, contrat Gaz, 1 procédure). Reste plus tard : budget/dépenses (`budgetV2`, vides sur les copros test) + contrat de syndic.
...
- **Prochaine action concrète** : eStale E1-E4 faits (CS, AG, construction/PPT/DPE, contrats, procédures). Suite au choix : **ODJ-4 mise en forme** du document (logo, typo, en-tête/pied), **budget/dépenses** eStale (quand des copros en auront), ou **contrat de syndic**. En fond : **compte de service eStale dédié** (auj. = compte perso Sekou, à remplacer pour la prod, ADR-005) ; vers la prod : `db pull` patron (3 tables) + Vercel ; Entra ID. Backlog : MYTHEC (facturation dépassement). (Expo, quand voulu) infra seulement : **`db pull` côté patron** (consigne prête, désormais pour **3 tables**) + **déploiement Vercel** (différé). (Plus tard) **Entra ID** (remplace le sélecteur dev + débloque mails/réservation de salle), durcissement (RLS si possible, tests E2E).
