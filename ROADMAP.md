...
- **Phase** : 5 écrans MVP + **ODJ** **branchés sur la vraie data** (264 copros + **3 tables natives**) **avec cloisonnement gestionnaire** (sélecteur dev + filtrage `managerId`). Reste : **auth réelle (Entra ID)** qui remplacera le sélecteur, + durcissement. Le mock demeure le fallback (`COPRO_SOURCE` absent).
...
- **Prochaine action concrète** : **INTÉGRATION ESTALE** (décision Sekou 2026-06-12 : gros chantier, J4 anticipé, cf. ADR-022). eStale = **source primaire** ; objectif immédiat : remplir les champs sourcés « estale » de l'ODJ (membres CS, comptes/budget, procédures, sinistres, contrats) + les blocs eStale de la fiche. Préalables : endpoint GraphQL + identifiants (compte de service / API key, cf. ADR-005). Ensuite (ODJ) : **ODJ-4 mise en forme du document** (typo, logo REAL31, en-tête/pied - composant à nous, stylable ; option ultérieure : modèle Word tagué), export Word si besoin. Backlog : MYTHEC (facturation dépassement). (Expo, quand voulu) infra seulement : **`db pull` côté patron** (consigne prête, désormais pour **3 tables**) + **déploiement Vercel** (différé). (Plus tard) **Entra ID** (remplace le sélecteur dev + débloque mails/réservation de salle), durcissement (RLS si possible, tests E2E).
