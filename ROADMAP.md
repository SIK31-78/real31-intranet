...
- **Derniers incréments terminés** :
  ...
  - ✅ **LIEN ESTALE <-> INTRANET BRANCHÉ EN RÉEL (2026-06-12, Phase B ADR-022, J4 anticipé)** : c'est le **socle de lecture** de la plateforme. Détails :
    ...
    - **Reste E4** : données **comptables/gestion** de l'ODJ (budget/dépenses via `accountingV2`/budgets, procédures `Litigation`, contrat de syndic) - partie exploratoire.
- **Bloqueurs / en attente** :
  - ⚠️ **Copros gagnées sur eStale absentes de la base Crypto/Supabase** : l'intranet lit le référentiel depuis Supabase ; une copro créée directement sur eStale (ex. la copro test **SE999 « 31 rue de l'Estale »**, ou les copros fraîchement gagnées hors Crypto) **n'apparaît pas** dans l'app tant qu'elle n'est pas ajoutée au référentiel. À trancher : ajout manuel d'une ligne `Copropriete` (write patron) vs création côté Crypto vs futur référentiel eStale-first.
...
- **Prochaine action concrète** : **eStale E4** = brancher les champs **comptables/gestion** de l'ODJ (budget/dépenses, procédures, contrat de syndic) - exploration du schéma sur les copros pilotes. En parallèle : rendre la **copro test SE999** testable dans l'app (cf. bloqueur). Plus tard : **ODJ-4 mise en forme** du document (logo, typo, en-tête/pied), export Word si besoin ; **compte de service eStale dédié** (aujourd'hui = compte perso Sekou, à remplacer pour la prod, ADR-005) ; vers la prod : `db pull` patron (3 tables) + Vercel ; Entra ID. Backlog : MYTHEC (facturation dépassement). (Expo, quand voulu) infra seulement : **`db pull` côté patron** (consigne prête, désormais pour **3 tables**) + **déploiement Vercel** (différé). (Plus tard) **Entra ID** (remplace le sélecteur dev + débloque mails/réservation de salle), durcissement (RLS si possible, tests E2E).
