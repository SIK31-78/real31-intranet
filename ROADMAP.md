...
- **Prochaine action concrète** : (contenu) **réintégrer la section ODJ** de la checklist supervision (Sekou expliquera comment). (Expo, quand voulu) infra seulement : **`db pull` côté patron** (consigne prête, à transmettre) + **déploiement Vercel** (différé). (Plus tard) **Entra ID** (remplace le sélecteur dev), durcissement (RLS si possible, tests E2E).
