...
- **Bloqueurs / en attente** :
  ...
  - 🔄 **SSO Microsoft Entra ID - code prêt (2026-06-16, J1b anticipé)** : **Auth.js v5** (ADR-017) branché - provider Microsoft Entra ID (`src/auth.ts`), route `/api/auth/[...nextauth]`, login -> email -> gestionnaire (`User.findByEmail`) -> cloisonnement `managerId` inchangé. **Fallback dev-login** tant que les identifiants Azure sont absents (rien ne casse). Le gate mot de passe se désactive quand le SSO est actif. Vars `AZURE_*` + `AUTH_SECRET`. **Bloqueur** : le patron doit créer l'App Registration et fournir `AZURE_TENANT_ID/CLIENT_ID/CLIENT_SECRET` (instructions ciblées « Étape 1 SSO » en tête de `docs/entra-app-registration.md`). Ensuite : poser les vars + tester le login réel.
- **Prochaine action concrète** : **SSO** = le patron crée l'App Registration Entra ID (note « Étape 1 » du doc) et nous transmet les 3 valeurs `AZURE_*` ; ensuite on les pose dans `.env.local` et on **teste le login réel**. ODJ : mise en forme **faite** (logo + titres verts), vérif comptes **complète** depuis eStale. Pistes restantes : section **gestion courante** (narratif manuel), reste eStale (vs N-1, fonds placés), **compte de service eStale dédié** (prod, ADR-005). (Expo) infra : `db pull` patron (3 tables) + Vercel. **Backlog** : MYTHEC (facturation dépassement) ; **gestion des dossiers sinistres sur l'intranet** (eStale n'a pas de module sinistre - décision Sekou 2026-06-16).
...
- 🔄 **App Registration Entra ID** (cf. `docs/entra-app-registration.md`, note « Étape 1 SSO » en tête) : **code SSO prêt** (Auth.js v5), il manque que le **patron crée l'app** et nous donne `AZURE_TENANT_ID/CLIENT_ID/CLIENT_SECRET`. Mail/SharePoint = étapes ultérieures.
