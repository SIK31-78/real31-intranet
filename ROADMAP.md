...
- **Prochaine action concrète** : eStale E1-E4 faits (CS, AG, PPT/DPE auto, contrats, **toute la vérification des comptes** : budget, dépenses, eau, fonds travaux, travaux votés/dépensés, débiteurs >5%). Suite au choix : **ODJ-4 mise en forme** du document (logo, typo, en-tête/pied), section **gestion courante** (sinistres, autres dossiers, ENGIE, DTG), ou reste eStale (vs N-1, fonds placés, contrat de syndic). En fond : **compte de service eStale dédié** (auj. = compte perso Sekou, à remplacer pour la prod, ADR-005). (Expo, quand voulu) infra : **`db pull` côté patron** (3 tables) + **Vercel** (différé). (Plus tard) **Entra ID** (sélecteur dev -> vraie auth, débloque mails/réservation de salle), durcissement (RLS si possible, tests E2E). Backlog : MYTHEC (facturation dépassement).
