  ...
  return (
    <div className="min-h-screen bg-canvas p-8">
      <div className="mx-auto max-w-[720px] bg-surface border border-line rounded-lg shadow-1 p-6">
        ...
        {result.ok ? (
          <>
            <div className="inline-flex items-center gap-2 px-2.5 h-6 rounded-sm bg-ok-50 text-ok-700 text-body font-medium border border-ok-500/30 mb-4">
              ...
              <table className="w-full text-body">
                <thead className="bg-surface-2/40">
                  <tr className="text-left text-ink-3">
                    <th className="px-3 py-2 font-medium text-meta uppercase tracking-[0.06em]">
                      Key
                    </th>
                    <th className="px-3 py-2 font-medium text-meta uppercase tracking-[0.06em]">
                      Description
                ...
                <tbody>
                  {result.rows.map((row) => (
                    ...
                    >
                      <td className="px-3 py-2 font-mono text-body text-ink-2">
                        {row.key}
                      </td>
          ...
          <div className="bg-err-50 border border-err-500/30 rounded-sm p-3">
            <div className="text-body font-medium text-err-700 mb-1">Erreur</div>
            <div className="font-mono text-body text-err-700 whitespace-pre-wrap break-all">
              {result.error}
            </div>
          </div>
        )}
