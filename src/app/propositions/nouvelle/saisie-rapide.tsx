...
// Saisie rapide d'une proposition : l'adresse cherche dans le registre national, le reste
// est facultatif. Ce qui manque pour faire l'offre est dit sous le formulaire (« quelle
// info prendre » pour celui qui décroche).
...
export function SaisieRapide({ agences, agenceParDefaut }: { agences: string[]; agenceParDefaut?: string }) {
  ...
  const manquant = informationsManquantes(apercu);
    ...
    <Card>
      <CardBody className="flex flex-col gap-5">
        <div className="flex flex-col gap-2">
          <Field label="Adresse de l'immeuble" htmlFor="np-adresse">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-4">
          ...
          <Field label="Téléphone" htmlFor="np-tel"><Input id="np-tel" value={telephone} onChange={(e) => setTelephone(e.target.value)} /></Field>
          <Field label="E-mail" htmlFor="np-email"><Input id="np-email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} /></Field>
        </div>
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-4">
          ...
          <Field label="Origine" htmlFor="np-origine">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-4">
          ...
          <Field label="Notes" htmlFor="np-notes" className="sm:col-span-3">
            <Textarea id="np-notes" rows={2} value={commentaires} onChange={(e) => setCommentaires(e.target.value)} placeholder="Ce qui a été dit au téléphone, ce que la personne attend…" />
          </Field>
        </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <p className="text-body text-ink-2">
            {manquant.length === 0 ? (
              <Badge ton="ok">Tout ce qu&apos;il faut pour faire l&apos;offre</Badge>
            ) : (
              <>À demander si possible : {manquant.join(", ")}.</>
            )}
          </p>
          <Button type="button" variant="primary" disabled={pending || !adresse.trim()} onClick={creer}>
