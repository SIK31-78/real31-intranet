...
- cles : {"code":str numerique,"libelle":str,"totalAttendu":int,"defaut":bool?,"commentaire":str?}. Une cle par nature de charges (001 charges generales, 100 batiment A, 210 ascenseur...).
- LIBELLE STRICT (limite eStale : 80 car MAX) : reprends LE TERME EXACT du RCP (nom de la colonne du tableau de repartition) ou, s'il existe, le libelle COURT de la ligne comptable du RGDD (ex. "CHAUFFAGE SANS LOT 1"). N'INVENTE PAS de paraphrase explicative (pas de "frais communs repartis...", pas de "(colonne 8)" etc. dans le libelle) : ces precisions vont dans "commentaire" (<=500 car), UNIQUEMENT si le libelle strict depasse 80 car (tres rare). Sinon omets "commentaire".
- CLE PAR DEFAUT : eStale cree AUTOMATIQUEMENT une cle "Charges generales" a la creation de la copro. Identifie CELLE des cles extraites qui correspond a cette quote-part generale (la clé de base concernant TOUS les lots, PAS une clé specifique comme ascenseur/eau/chauffage meme si elle couvre aussi tous les lots) et marque-la "defaut":true. Une SEULE cle peut porter defaut:true.
...
- UNE CLE NE NAIT QUE D'UN TABLEAU DE TANTIEMES (regle CARDINALE, corrigee le 2026-07-30). Tu ne crees une cle QUE si tu vois, dans le RCP / l'EDD / une annexe, un TABLEAU DE REPARTITION DONNANT UN TANTIEME PAR LOT. Une colonne de charges vue UNIQUEMENT en comptabilite (RGD, annexes, budget), SANS tableau de tantiemes par lot, N'EST PAS une cle de repartition : signale-la en NOTE ("cle comptable X vue au RGD sans tableau de tantiemes -- module compteurs ou imputation directe, hors patrimoine") et n'emets AUCUNE cle. Cas typiques a NE PAS transformer en cles : les cles compteurs (eau chaude/froide au compteur), les consommations privatives, les charges refacturees a l'unite.
- totalAttendu NE PEUT VENIR QUE de l'en-tete ou du pied d'un TABLEAU DE TANTIEMES (le "100000", le "10000", le total imprime de la colonne). JAMAIS d'un budget, d'un total de depenses, d'un montant en euros, ni d'un volume (m3 d'eau). Si tu ne trouves pas de total imprime pour un tableau, omets la cle et signale-le en note : une cle dont le total est inconnu ne peut pas etre verifiee, donc elle ne sert a rien.
- CLES REELLES (comptabilite), dans les limites ci-dessus : si des ANNEXES COMPTABLES / RGD (releve general des depenses) / etats de repartition / budget de la convocation sont fournis, ils FONT FOI sur QUELLES cles sont reellement utilisees en compta -- c'est-a-dire sur le PERIMETRE (laquelle existe, laquelle est eclatee en deux), jamais sur les tantiemes eux-memes. Une charge regroupee en UNE seule colonne au RCP peut y etre ECLATEE en plusieurs cles (ex. eau froide vs eau chaude/chauffage) : cree alors DEUX cles distinctes -- mais SEULEMENT si tu disposes d'un tableau de tantiemes pour chacune ; sinon UNE cle + note. A l'inverse ne fusionne pas deux cles que la compta separe. Sans document comptable : se baser sur le tableau RCP et SIGNALER l'ambiguite (fusion possible a confirmer) en note.
...
- HOMONYMES (R6) : TU NE FUSIONNES JAMAIS DEUX LIGNES. Tu PROPOSES, en note ; la fusion est un geste HUMAIN dans l'editeur de corrections. Deux lignes de meme nom+prenom -> emets DEUX owners et signale "fusion a confirmer" en note. Corrige le 2026-07-30 : la formule "sans element distinctif" laissait le modele fusionner de sa propre initiative, et deux copropriétaires distincts ont ete perdus (43 owners au lieu de 44 sur S0306) -- une fusion faite a l'extraction est IRRECUPERABLE en aval, personne ne peut deviner qu'une personne manque.
- ELEMENTS DISTINCTIFS (liste NON exhaustive, tout element suffit a affirmer deux personnes) : civilite differente (Mme vs Mlle vs M.), ADRESSE differente, LOTS detenus differents, date ou lieu de naissance, email, SIREN. Dans le doute, TOUJOURS deux owners + note : garder deux lignes de trop se corrige d'un clic, une personne perdue ne se voit pas.
