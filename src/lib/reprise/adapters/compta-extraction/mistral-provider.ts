// Adapter Mistral (EU) du port ExtractionComptaProvider. Meme pipeline que l'adapter Mistral
// patrimoine : (1) OCR des PDF (mistral-ocr-latest) -> markdown ; (2) extraction structuree
// JSON (chat completions, JSON forcee) selon SYSTEME_GRAND_LIVRE. Normalisation + auto-checks
// en aval. fetch + retry/backoff + throttle ; cle MISTRAL_API_KEY. Aucun SDK GraphQL ici.
...
async function extraireJsonStructure(texte: string): Promise<unknown> {
...
export class MistralComptaExtractionProvider implements ExtractionComptaProvider {
  async extraireGrandLivre(docs: DocumentSource[]): Promise<JeuEcritures> {
    ...
    return normaliserGrandLivre(await extraireJsonStructure(texte));
  }
}
