...
/**
 * Evenements d'UNE copro derives de ses 4 dates cles (prochaine/derniere AG & CS). Pur (aucun
 * fetch) : sert au calendrier (boucle sur le portefeuille) ET a la fiche copro (une seule copro,
 * sans recharger tout le portefeuille). `confAg`/`confCs` = confirmations deja lues (statut des
 * chips "à confirmer" / "confirmée" sur les dates a venir).
 */
export function evenementsDeCopro(
  c: Copropriete,
  confAg: ConfirmationEvenement | null,
  confCs: ConfirmationEvenement | null,
  today: string,
): Evenement[] {
  const base = { coproCode: c.code, coproNomCourt: c.nom };
  const evs: Evenement[] = [];
  // Regle patron : une AG PASSEE dont la date tombe dans l'exercice comptable en cours est
  // invisible sur le planning (elle a eu lieu, la prochaine sera celle de l'exercice suivant).
  if (c.prochaineAg && !agMasqueeCarTenue(c.prochaineAg.date, c.exercice, today)) {
    ...
    evs.push({
      ...
      ...(c.prochaineAg.date >= today
        ? { confirmation: statutPourDate(confAg, c.prochaineAg.date) }
        : {}),
    });
  }
  if (c.prochaineCsDate) {
    evs.push({
      ...
      ...(c.prochaineCsDate >= today
        ? { confirmation: statutPourDate(confCs, c.prochaineCsDate) }
        : {}),
    });
  }
...
async function composerEvenementsReels(managerId: string): Promise<Evenement[]> {
  ...
  for (const c of copros) {
    evs.push(
      ...evenementsDeCopro(
        c,
        confParCle.get(`${c.code}__AG`) ?? null,
        confParCle.get(`${c.code}__CS`) ?? null,
        today,
      ),
    );
  }
