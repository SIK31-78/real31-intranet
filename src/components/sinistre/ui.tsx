...
export function References({ items }: { items?: string[] }) {
  ...
  return <p className="mt-3 text-meta text-ink-3">Références : {items.join(' · ')}</p>;
}
...
export function AlertBox({ children }: { children: ReactNode }) {
  return (
    <div
      role="note"
      className="mt-3 flex gap-2 rounded-md border-l-4 border-warn-500 bg-warn-50 p-3 text-body text-warn-700"
    >
...
export function GlosedList({ items, className = '' }: { items: string[]; className?: string }) {
  return (
    <ul className={`list-disc space-y-1 pl-5 text-body text-ink-2 ${className}`}>
      {items.map((it, i) => (
        <li key={i}>
          <Glose>{it}</Glose>
        </li>
...
export function SectionTitle({ children }: { children: ReactNode }) {
  return (
    <h3 className="text-meta font-medium uppercase tracking-[0.06em] text-ink-3">{children}</h3>
  );
}
