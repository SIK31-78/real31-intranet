...
export function CoproHeader({ copro }: { copro: Copropriete }) {
  ...
  return (
    <div className="bg-surface border border-line rounded-md p-4 sm:p-5 flex flex-col sm:flex-row items-start gap-4">
      ...
      {copro.source === "estale" && copro.estaleDeepLink && (
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="shrink-0 inline-flex items-center justify-center gap-1.5 h-8 px-3 rounded-md border border-line bg-surface text-[13px] text-ink hover:bg-surface-2 transition-colors w-full sm:w-auto"
        >
