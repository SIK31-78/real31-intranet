...
export function FluxActivite({ activite }: { activite: ItemActivite[] }) {
  return (
    <Card>
      ...
      <div className="px-4">
        {activite.length === 0 ? (
          // Etat vide : annonce explicite pour les lecteurs d'ecran
          <div
            role="status"
            aria-live="polite"
            className="flex flex-col items-center gap-2 py-8 text-ink-3"
          >
            <Inbox className="w-6 h-6 opacity-40" strokeWidth={1.5} aria-hidden={true} />
            <span className="text-[13px]">Aucune activité récente</span>
          </div>
        ) : (
          activite.map((item) => (
